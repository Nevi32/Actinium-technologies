...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        foreach ($profitData as $profit) {
            $totalStoreProfit += $profit['Profit'];
        }

        // Fetch total expenses for the day
        $expensesStmt = $pdo->prepare("SELECT SUM(amount) AS total_expenses FROM expenses WHERE DATE(timestamp) = CURDATE()");
        $expensesStmt->execute();
        $expensesData = $expensesStmt->fetch(PDO::FETCH_ASSOC);
        $totalExpenses = $expensesData['total_expenses'];

        // Calculate true profits (profits after subtracting expenses)
        $trueProfit = $totalStoreProfit - $totalExpenses;

        // Construct true profits report
        $trueProfitReport = array(
            'Total Store Profit' => $totalStoreProfit,
            'Total Expenses' => $totalExpenses,
            'True Profit' => $trueProfit
        );

        // Encode the response as JSON and return it
        echo json_encode(array('ProfitData' => $profitData, 'TrueProfitReport' => $trueProfitReport));
    } catch (Exception $e) {
